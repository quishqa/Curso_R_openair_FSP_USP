...
* Analizadores de poluentes mendem concentrações com maior frequência.
...
## Vetores
- Uma \alert{sequência} é definida **`seq(inicio, final, intervalo)`**
```{r, echo=T}
de_1ate5 <- seq(1, 5)
de_1ate5
...
## Vetores
- Para \alert{selecionar} elementos do vetor: **`nombre_vetor[posição]`**:
```{r, echo=T}
# Primeiro elemento
...
- Podemos \alert{selecionar} varios elementos usando **`outro vetor`**
```{r , echo=T}
# Segundo y tercero
...
- Vamos ler dados do ano 2023 do aeroporto de Guraulhos.
...
- No R para leer tabelas em csv (e para outros formatos) usamos a função \alert{`read.table()`}.
- Esta função precisa saber a **ubicação do arquivo**. Para isso usamos a função **`file.choose()`**.
...
## `data frames` Ler arquivos `.csv`
- Exploramos o conteúdo do data frame:
```{r}
names(gru)
```
...
- Vamos olhar os estatísticos básicos usando `summary()` da temperatura, velocidade do vento, umidade do ar.
```{r}
