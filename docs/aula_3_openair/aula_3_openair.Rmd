...
```{r, eval=FALSE}
dado$date <- as.POSIXct(strptime(dado$date, format = "%Y-%m-%d %H:%M:%OS", 
                                 tz = "UTC"))
...
```{r, eval=FALSE}
diario <- timeAverage(dado, avg.time = "day") 
write.csv(dado, "C:/Users/Rafaela/Dropbox/POSDOC/curso rstudio/dado_teste.csv", 
          row.names = FALSE)
```
...
obs: pode ser usado o “selectByDate” nessa função

```{r, eval=FALSE}
timePlot(dado, 
         pollutant = c("o3","pm25"))

timePlot(selectByDate(dado, year = "2021"), #selecionando o período
...
```{r, eval=FALSE}
timePlot(selectByDate(dado, year = "2021", month = c("jun", "jul", "ago")),
         pollutant = c("pm25"), 
         date.format = "%b-%Y",
         avg.time = "day",
         lwd = 2,
...
```{r, eval=FALSE}
calendarPlot(dado, pollutant = "o3", year = 2021)

calendarPlot(selectByDate(dado, 
             year = 2021, month = c("jun","jul","ago")), #seleciona o ano/mês desejado
...
timeVariation(selectByDate(dado, year = 2021, month = c("jun", "julho", "ago")),
              pollutant = c("o3"), 
              group = "season",
              ylab = " O3 (ug m-3)", 
              hemisphere = "southern",
...
obs: deixar como nome das variáveis de “ws” e “wd”.
...
pollutionRose (selectByDate(dado, year = 2021),
               pollutant = "o3", 
               type = "season",
               hemisphere = "southern",
               layout = c(2,2))
...
Selecionar o período de 01 a 22 de março de 2020 (pré-lockdwon) e de 23 de março a 13 de abril de 2020 (Lockdown). Plotar a série temporal e verificar se ocorreu diferença de concentração entre os períodos.Avaliar também se houve diferença entre as concentrações de O3 e PM2.5 quanto ao perfil diurno e dia da semana. 
